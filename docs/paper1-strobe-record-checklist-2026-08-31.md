...
1. Freeze and cite the append-only `paper1-baseline-v1` analysis snapshot. The
   ...
   [`createAnalysisSnapshot`](../lib/data/snapshots.ts#L46)).
2. Bind headline-number verification to that frozen payload. The existing
   [`verify-paper-figures.mjs`](../scripts/verify-paper-figures.mjs#L68) checks
   the mutable registry, not a selected snapshot.
...
5. Do not copy headline figures from the older v0.1 findings memo. It retains
   ...
   [`verify-paper-figures.mjs`](../scripts/verify-paper-figures.mjs#L68) and the
...
| 1b — balanced abstract | PARTIAL | Headline sample and outcome assertions are enumerated in [`CHECKS`](../scripts/verify-paper-figures.mjs#L68), while the readiness audit records no frozen snapshot ([evidence census](paper1-submission-readiness-2026-08-31.md#evidence-census)). | Write a structured abstract covering source, period, selection, descriptive methods, principal missingness result, limitations, and conclusion; populate numbers only from the frozen release. |
...
| 5 — setting and dates | DONE | Location, source purpose, period (2 Dec 2018–24 Mar 2020), COVID stop, and corridor concentration appear in [findings §§1–2](paper1-baseline-findings.md#1-sample); exact boundary assertions are in [`CHECKS`](../scripts/verify-paper-figures.mjs#L68). | Transcribe the dates and distinguish operational data-collection dates from the later ETL/reconciliation dates. State that the registry served tourist medical coordination rather than research collection. |
