...
**Multi-leg journey assumption**: a case that transits through two facilities — for example, a tourist evacuated from a remote clinic to a tertiary hospital — has two `FACILITY_ARRIVAL` events at different `occurred_at` times. The metric uses the first. The database deduplicates only on the full tuple `(case_id, event_type, occurred_at, actor_id)` (UNIQUE constraint + `ON CONFLICT DO NOTHING` in `upsert_case_event`, migration `20260519_006`). This is deliberately loose enough to *permit* multiple same-type events at distinct times (the two `FACILITY_ARRIVAL` legs) while still blocking exact-duplicate re-inserts from concurrent triggers or re-run backfill batches. The "use the first" rule therefore lives in application code (`findEvent`), not in a one-row-per-type DB constraint.

**Methods-section paraphrase**: "When multiple events of a given type were recorded for a case, the earliest by `occurred_at` was used for metric computation. For example, a case with two `FACILITY_ARRIVAL` events (initial clinic and subsequent tertiary facility) contributed only the first to its TTDC calculation. Exact-duplicate events were prevented by a database uniqueness constraint on `(case_id, event_type, occurred_at, actor_id)`; selection of the earliest event of a type was performed at the application level (linear scan returning the first match in chronologically-sorted event arrays)."
