...
| `funding_opportunities` | `source_url` required. Classify `eligibility_category` honestly: `a_open_now` (independent applicants accepted today), `c_company_eligible` (Tourist SOS applies as an organisation), `b_needs_affiliation` (only after a PhD place). | The owner is pre-acceptance with no academic affiliation. Misfiling an affiliation-gated grant as open-now wastes real time on an application that gets rejected unread. |
